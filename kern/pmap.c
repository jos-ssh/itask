...
alloc_child(struct Page *parent, bool right) {
    ...
    assert(!(parent->addr & ((1 << parent->class) - 1)));
...
page_lookup(struct Page *hint, uintptr_t addr, int class, enum PageState type, bool alloc) {
    ...
    if (alloc && type != PARTIAL_NODE) /* Attach new memory */ {
        ...
        if (node->state == ALLOCATABLE_NODE) {
            list_append(&free_classes[node->class], (struct List *)node);
        }
...
static size_t
get_max_class(uintptr_t start, uintptr_t end) {
    if (end < start || end - start < CLASS_SIZE(0)) {
        panic("Cannot assign class to region [%lu, %lu]", start, end);
    }
    for (size_t class = MAX_CLASS; class > 0; --class) {
        if (end - start >= CLASS_SIZE(class) &&
            (start & CLASS_MASK(class)) == 0) {
            return class;
        }
    }
    return 0;
...
attach_region(uintptr_t start, uintptr_t end, enum PageState type) {
    if (trace_memory_more)
        cprintf("Attaching memory region [%08lX, %08lX] with type %X\n", start, end - 1, type);
    ...
    while (start < end) {
        int class = get_max_class(start, end);
...
static void
dump_virtual_tree_recursive(struct Page *node, int class, uintptr_t vaddr,
                            size_t depth) {
    ...
    if (node->state & PROT_LAZY) cprintf(", LAZY");
    if (node->state & PROT_COMBINE) cprintf(", COMBINE");
    ...
    if (node->state & MAPPING_NODE) {
        ...
        cprintf("|> mapped from paddr[0x%lx, 0x%lx)\n", pstart, pend);
    }

    if (class > 0) {
        if (node->left) {
            dump_virtual_tree_recursive(node->left, class - 1, vaddr, depth + 1);
        }
        if (node->right) {
            dump_virtual_tree_recursive(node->right, class - 1,
...
dump_virtual_tree(struct Page *node, int class) {
    assert(class >= 0);
    uintptr_t vaddr = 0;
    size_t node_class = class;
    while (node->parent) {
        assert(node_class > 0);
        if (node == node->parent->right) {
            vaddr += CLASS_SIZE(node_class);
        }
        ++node_class;
        node = node->parent;
    }
    assert(node_class == MAX_CLASS);

    dump_virtual_tree_recursive(node, class, vaddr, 0);
}
...
dump_page_table(pte_t *pml4) {
    cprintf("Page table:\n");
    for (size_t pml4_i = 0; pml4_i < PML4_ENTRY_COUNT; ++pml4_i) {
        pte_t pml4_entry = pml4[pml4_i];
        if (!(pml4_entry & PTE_P)) {
            continue;
        }

        dump_entry(pml4_entry, 512 * GB, false);
        pte_t *pdp = (pde_t *)KADDR(PTE_ADDR(pml4_entry));

        for (size_t pdp_i = 0; pdp_i < PDP_ENTRY_COUNT; ++pdp_i) {
            pte_t pdp_entry = pdp[pdp_i];
            if (!(pdp_entry & PTE_P)) {
                continue;
            }

            bool is_leaf = !!(pdp_entry & PTE_PS);
            dump_entry(pdp_entry, 1 * GB, is_leaf);
            if (is_leaf) {
                continue;
            }

            pte_t *pd = (pte_t *)KADDR(PTE_ADDR(pdp_entry));
            for (size_t pd_i = 0; pd_i < PD_ENTRY_COUNT; ++pd_i) {
                pte_t pd_entry = pd[pd_i];
                if (!(pd_entry & PTE_P)) {
                    continue;
                }

                bool is_leaf = !!(pd_entry & PTE_PS);
                dump_entry(pd_entry, 2 * MB, is_leaf);
                if (is_leaf) {
                    continue;
                }

                pte_t *pt = (pte_t *)KADDR(PTE_ADDR(pd_entry));
                for (size_t pt_i = 0; pt_i < PT_ENTRY_COUNT; ++pt_i) {
                    pte_t pt_entry = pt[pt_i];
                    if (!(pt_entry & PTE_P)) {
                        continue;
                    }

                    dump_entry(pd_entry, 4 * KB, true);
                }
            }
        }
    }
...
map_page(struct AddressSpace *spc, uintptr_t addr, struct Page *page, int flags) {
    ...
    /* If page directory is not present */
    if (!(pd[pdi0] & PTE_P)) {
 ...
 * (Why it might me impactful? - Loading of CR3 invalidates TLB. This wouldn't
 * happen if we used PCIDs)
 *
...
detect_memory(void) {
    ...
    attach_region((unsigned long)IOPHYSMEM,
                  (unsigned long)PADDR(end),
                  RESERVED_NODE);

    unsigned long kern_end_paddr = (unsigned long)PADDR(end);
    ...
    if (uefi_lp && uefi_lp->MemoryMap) {
        ...
        while (start < end) {
            enum PageState type;
            size_t block_size = start->NumberOfPages * EFI_PAGE_SIZE;
            switch (start->Type) {
            ...
            EFI_PHYSICAL_ADDRESS start_paddr = start->PhysicalStart;
            EFI_PHYSICAL_ADDRESS end_paddr = start_paddr + block_size;
            assert(start_paddr < end_paddr);

            max_memory_map_addr = MAX(end_paddr, max_memory_map_addr);

            /* If region is not reserved */
            if (start_paddr >= (unsigned long)UTEMP &&
                (IOPHYSMEM > start_paddr || start_paddr >= kern_end_paddr)) {

                /* Attach memory described by memory map entry
                 * described by start of type type */
                attach_region(start_paddr, end_paddr, type);
            }
...
init_memory(void) {
    ...
    //     [FRAMEBUFFER, FRAMEBUFFER + uefi_lp->FrameBufferSize] as RW- + PROT_WC
    res = map_physical_region(&kspace,
                              FRAMEBUFFER,
    ...
    res = map_physical_region(&kspace,
                              ...
                              PADDR(__text_start),
                              ROUNDUP(__text_start - __text_end, CLASS_SIZE(0)),
                              PROT_R | PROT_X);
    assert(!res);
